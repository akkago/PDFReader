...
describe('codes2.txt no-extra coverage', () => {
  test('никакие коды из codes2.txt, отсутствующие в raw2.txt, не появляются в parsePageContent', () => {
    ...
    const codesInRaw = codes.filter(code => allStrings.includes(code));
    expect(codesInRaw.length).toBeGreaterThan(0);
    ...
    const codesNotInRaw = codes.filter(code => !allStrings.includes(code));
